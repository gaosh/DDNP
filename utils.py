...
class Flops_constraint_resnet_k(nn.Module):
    def __init__(self, p, kernel_size, out_size, group_size, size_inchannel, size_outchannel, in_channel=3, w=2, structure=None, loss_type='log', ar_v=True):

        super(Flops_constraint_resnet_k, self).__init__()
    ...
    def forward(self, input, ad_input=None):
        ...
        for i in range(length):
            ...
            if self.ar_v:
                self.input = input
                if ad_input is not None:

                    c_out = input[i].sum() + self.structure[i] *ad_input[i]
                    c_out = c_out/2
                else:
                    c_out = input[i].sum() + self.structure[i] * ad_input[i]
                ...
                self.input = input.squeeze()
                input = input.squeeze()
                c_out = self.structure[i] * input[i]
        ...
        if self.loss_type == 'log':
            ...
            if resource_ratio - (self.p) >= 0:
                loss = torch.log((resource_ratio / (self.p)))
            else:
                loss = torch.log(((self.p) / resource_ratio ))
...
class Flops_constraint_mobnet_k(nn.Module):
    def __init__(self, p, kernel_size, out_size, group_size, size_inchannel, size_outchannel, w=2, structure=None):
        super(Flops_constraint_mobnet_k, self).__init__()
    ...
    def print_current_FLOPs(self, input):
        ...
        with torch.no_grad():
            for i in range(length):
                channels = self.structure[i]*input[i]
                #two layer as a group
...
def print_model_param_flops(model=None, input_res=224, multiply_adds=False, given_ins=None):
    prods = {}
    ...
    def foo(net):
        ...
        if not childrens:
            ...
            if isinstance(net, torch.nn.GRU):
                net.register_forward_hook(rnn_flops_counter_hook)
            return
        for c in childrens:
            foo(c)
    ...
    total_flops = (sum(list_conv) + sum(list_linear) + sum(list_bn) + sum(list_relu) + sum(list_pooling) + sum(
        list_upsample) + sum(list_rnn))

    if given_ins is not None:
        print('  + Number of FLOPs: %.5fG' % (total_flops / 3 / 1e9))
    else:
        print('  + Number of FLOPs: %.5fG' % (total_flops / 1 / 1e9))
